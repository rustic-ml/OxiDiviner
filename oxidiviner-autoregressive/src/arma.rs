...
impl Forecaster for ARMAModel {
    ...
    fn forecast(&self, horizon: usize) -> Result<Vec<f64>> {
        ...
        if self.ar_coefficients.is_none() || self.ma_coefficients.is_none() || 
           self.last_values.is_none() || self.last_residuals.is_none() {
        ...
        for _ in 0..horizon {
            ...
            for i in 0..self.q {
                if i < extended_residuals.len() {
                    prediction += ma_coeffs[i] * extended_residuals[extended_residuals.len() - i - 1];
                }
            }
...
mod tests {
    ...
    fn test_arma_model_constant_data() {
        ...
        for forecast in forecasts {
            assert!((forecast - 10.0).abs() < 1e-5, 
                   "Forecast {} should be close to 10.0 for constant data", forecast);
        }
    }
    ...
    fn test_arma_model_trending_data() {
        ...
        for (i, forecast) in forecasts.iter().enumerate() {
            ...
            assert!((forecast - expected).abs() < 2.0, 
    ...
    fn test_arma_model_coefficient_access() {
        ...
        assert_eq!(ar_coefficients.len(), 2, "ARMA(2,1) model should have 2 AR coefficients");
        
        // Should be able to access MA coefficients
        let ma_coefficients = model.ma_coefficients().unwrap();
        assert_eq!(ma_coefficients.len(), 1, "ARMA(2,1) model should have 1 MA coefficient");
