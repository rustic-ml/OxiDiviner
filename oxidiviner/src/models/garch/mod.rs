...
mod tests {
    ...
    fn test_gjr_garch_advanced() {
        ...
        for i in 0..n {
            if i > 0 {
                ...
                let asymmetric_term = if prev_return < 0.0 {
                    0.05 * prev_return.powi(2)
                } else {
                    0.0
                };
    ...
    fn test_gjr_garch_asymmetric_effects() {
        ...
        let positive_idx = shocks
            .iter()
            .rposition(|&x| x > 1.0)
            .unwrap_or(shocks.len() - 1);
    ...
    fn test_garch_fitting_comprehensive() {
        let data_sets = [
            ...
            (0..100)
                .map(|i| (i as f64 * 0.05).sin() * 0.05)
                .collect::<Vec<f64>>(),
            // Low volatility data
            (0..100)
                .map(|i| (i as f64 * 0.2).cos() * 0.001)
                .collect::<Vec<f64>>(),
        ];
        ...
        for (i, data) in data_sets.iter().enumerate() {
            ...
            let mut garch_m_model =
                GARCHMModel::new(1, 1, RiskPremiumType::Variance, None).unwrap();
    ...
    fn test_different_risk_premium_types() {
        let data: Vec<f64> = (0..50)
            .map(|i| 0.001 + (i as f64 * 0.1).sin() * 0.01)
            .collect();
