...
impl GaussianCopulaModel {
    ...
    fn fit_garch_marginal(&mut self, series_idx: usize, data: &[f64]) -> Result<()> {
        ...
        for i in 1..n {
            returns.push((data[i] / data[i - 1] - 1.0) * 100.0); // Convert to percentage returns
        }
    ...
    pub fn forecast_multivariate(
        ...
        for series_idx in 0..self.n_series {
            ...
            let _long_run_var = omega / (1.0 - alpha - beta);
...
impl GaussianCopulaModel {
    ...
    pub fn fit_multivariate(&mut self, data: &[Vec<f64>]) -> Result<()> {
        ...
        for i in 0..self.n_series {
            self.fit_garch_marginal(i, &series_data[i])?;
...
mod tests {
    ...
    fn generate_test_data() -> Vec<Vec<f64>> {
        ...
        for t in 1..n_obs {
            ...
            data[t][0] = 100.0 + t_f * 0.1 + (t_f * 0.2).sin() * 2.0;
            // Series 2: correlated with series 1 but with different volatility
            data[t][1] = 200.0 + t_f * 0.05 + (t_f * 0.15).cos() * 3.0 + data[t][0] * 0.1;
        }
