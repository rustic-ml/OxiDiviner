...
impl RegimeDetector {
    ...
    pub fn new(config: AdaptiveConfig) -> Result<Self> {
        ...
        let data_window_size = 100; // Fixed window size for stability
    ...
    pub fn fit(&mut self, data: &TimeSeriesData) -> Result<()> {
        ...
        let min_window = 50; // Minimum required for regime detection
        if data.values.len() < min_window {
        ...
        let window_size = 100; // Fixed window size
    ...
    pub fn detect_regime(&mut self, new_value: f64) -> Result<RegimeDetectionResult> {
        ...
        // Create dummy timestamps for the window data
        use chrono::{Duration, Utc};
        let now = Utc::now();
        let timestamps: Vec<chrono::DateTime<Utc>> = (0..window_data.len())
            .map(|i| now - Duration::seconds(i as i64))
            .rev()
            .collect();

        let ts_data = TimeSeriesData::new(timestamps, window_data, "regime_detection")?;
        ...
        let current_regime =
            MarketRegime::from_index(regime_index, self.config.regime_config.num_regimes);
    ...
    pub fn validate_performance(
        ...
        // Create dummy timestamps for validation data
        use chrono::{Duration, Utc};
        let now = Utc::now();
        let fit_timestamps: Vec<chrono::DateTime<Utc>> = (0..fit_size)
            .map(|i| now - Duration::seconds(i as i64))
            .rev()
            .collect();
        ...
        for (&value, &true_regime) in validation_data.values[fit_size..]
            .iter()
...
impl RegimeDetectorBuilder {
    ...
    /// Set detection window size (note: currently uses fixed 100-point window)
    pub fn with_window_size(self, _window_size: usize) -> Self {
        // Currently using fixed window size, parameter kept for API compatibility
        self
    }
...
mod tests {
    ...
    fn test_regime_detector_builder() {
        ...
        // Window size is currently fixed at 100
    }
}
