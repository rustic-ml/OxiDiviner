...
fn generate_test_data(n: usize, with_trend: bool, with_seasonality: bool, seed: u64) -> TimeSeriesData {
...
fn test_moving_average_model() {
    ...
    let output = ma_model.predict(forecast_horizon, Some(&test_data)).expect("Failed to make predictions");
    ...
    assert!(eval.mae > 0.0 && eval.mae < 30.0, "MAE should be reasonable, got {}", eval.mae);
    assert!(eval.rmse > 0.0 && eval.rmse < 40.0, "RMSE should be reasonable, got {}", eval.rmse);
}
...
fn test_simple_exponential_smoothing_model() {
    ...
    // Make predictions
    let forecast_horizon = test_data.len();
    let output = ses_model.predict(forecast_horizon, Some(&test_data)).expect("Failed to make predictions");
    ...
    // Check that the MAE and RMSE are reasonable
    let eval = output.evaluation.unwrap();
    assert!(eval.mae > 0.0 && eval.mae < 20.0, "MAE should be reasonable, got {}", eval.mae);
    assert!(eval.rmse > 0.0 && eval.rmse < 25.0, "RMSE should be reasonable, got {}", eval.rmse);
}
...
fn test_holt_winters_model() {
    ...
    // Create and fit the model
    let mut hw_model = HoltWintersModel::new(0.2, 0.1, 0.1, 7).expect("Failed to create Holt-Winters model");
    hw_model.fit(&train_data).expect("Failed to fit Holt-Winters model");
    
    // Make predictions
    let forecast_horizon = test_data.len();
    let output = hw_model.predict(forecast_horizon, Some(&test_data)).expect("Failed to make predictions");
    ...
    // Check that the MAE and RMSE are reasonable
    let eval = output.evaluation.unwrap();
    assert!(eval.mae > 0.0 && eval.mae < 25.0, "MAE should be reasonable, got {}", eval.mae);
    assert!(eval.rmse > 0.0 && eval.rmse < 30.0, "RMSE should be reasonable, got {}", eval.rmse);
}
...
fn test_autoregressive_model() {
    ...
    // Make predictions
    let forecast_horizon = test_data.len();
    let output = ar_model.predict(forecast_horizon, Some(&test_data)).expect("Failed to make predictions");
    ...
    assert!(eval.rmse > 0.0, "RMSE should be positive, got {}", eval.rmse);
}
...
fn test_ensemble_forecasting() {
    ...
    // Generate forecasts
    let ma_output = ma_model.predict(forecast_horizon, Some(&test_data)).expect("Failed to make MA predictions");
    let ses_output = ses_model.predict(forecast_horizon, Some(&test_data)).expect("Failed to make SES predictions");
    let hw_output = hw_model.predict(forecast_horizon, Some(&test_data)).expect("Failed to make Holt-Winters predictions");
    ...
    for i in 0..forecast_horizon {
        ensemble_forecast[i] = (ma_output.forecasts[i] + ses_output.forecasts[i] + hw_output.forecasts[i]) / 3.0;
    }
    ...
    assert!(ensemble_mae > 0.0, "Ensemble MAE should be positive, got {}", ensemble_mae);
    assert!(ensemble_rmse > 0.0, "Ensemble RMSE should be positive, got {}", ensemble_rmse);
    ...
    assert!(ensemble_mae <= worst_model_mae * 1.2, "Ensemble should not be much worse than the worst model");
}
