...
fn main() -> oxidiviner::Result<()> {
    ...
    let timestamps: Vec<DateTime<Utc>> = (0..60).map(|i| start_date + Duration::days(i)).collect();
    ...
    println!(
        "Generated {} data points with trend and noise",
        values.len()
    );
    println!(
        "Data range: {:.2} to {:.2}",
    ...
    for (data, label) in &[
        (low_noise_data, "Low Noise"),
        (high_noise_data, "High Noise"),
    ] {
    ...
    println!(
        "Quick API forecast (default window, first 5 values): {:?}",
        &quick_forecast[..5]
    );
    ...
    println!(
        "Quick API forecast (window=7, first 5 values): {:?}",
        &quick_forecast_7[..5]
    );
    ...
    let sales_data: Vec<f64> = vec![
        120.0, 135.0, 142.0, 128.0, 155.0, 163.0, 178.0, 145.0, 160.0, 172.0, 185.0, 158.0, 195.0,
        202.0, 180.0, 215.0, 225.0, 198.0, 240.0, 255.0,
    ];
    ...
    for window in [3, 5, 7] {
        ...
        println!(
            "  Window {}: Next 5 days average sales = {:.1}",
            window,
            sales_forecast.iter().sum::<f64>() / sales_forecast.len() as f64
        );
    }
...
mod tests {
    ...
    fn test_moving_average_example() {
        ...
        assert!(
            result.is_ok(),
            "Moving Average example should run successfully: {:?}",
            result
        );
    }
}
