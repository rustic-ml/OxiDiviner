...
fn main() -> oxidiviner::Result<()> {
    ...
    let timestamps: Vec<DateTime<Utc>> = (0..500)
    ...
    let mut volatility = 0.02; // Initial volatility (2%)
    ...
    println!("Generated {} financial returns with volatility clustering", returns.len());
    println!("Return statistics:");
    println!("  Mean return: {:.4}", returns.iter().sum::<f64>() / returns.len() as f64);
    println!("  Volatility (std): {:.4}", {
        let mean = returns.iter().sum::<f64>() / returns.len() as f64;
        let variance = returns.iter().map(|r| (r - mean).powi(2)).sum::<f64>() / returns.len() as f64;
        variance.sqrt()
    });
    println!("  Min return: {:.4}", returns.iter().fold(f64::INFINITY, |a, &b| a.min(b)));
    println!("  Max return: {:.4}", returns.iter().fold(f64::NEG_INFINITY, |a, &b| a.max(b)));
    ...
    println!("\nData split: {} training, {} testing observations", 
             train_returns.len(), test_returns.len());
    ...
    println!("✓ Generated {} volatility forecasts", volatility_forecast.len());
        ...
        match GARCHModel::new(p, q, None) {
            Ok(mut model) => {
                match model.fit(&train_returns.values, Some(&train_returns.timestamps)) {
                    Ok(_) => {
                        match model.forecast_variance(test_returns.len()) {
                            Ok(vol_forecast) => {
                                let pred_vars: Vec<f64> = vol_forecast.iter().map(|v| v.powi(2)).collect();
                                let vol_mse = actual_squared_returns.iter()
                                    .zip(&pred_vars)
                                    .map(|(actual, pred)| (actual - pred).powi(2))
                                    .sum::<f64>() / actual_squared_returns.len() as f64;
                                println!("  {}: MSE = {:.6}, Avg Vol = {:.4}", 
                                        description, vol_mse, 
    ...
    for (i, &vol) in future_volatility.iter().take(10).enumerate() {
        println!("  Day {}: {:.4} ({:.2}% annualized)", 
                i + 1, vol, vol * (252.0_f64).sqrt() * 100.0); // Annualized volatility
    }
    ...
    for &confidence in &confidence_levels {
        ...
        println!("  {:.1}% VaR (1-day): ${:.0}", confidence * 100.0, var_1_day);
    }
    ...
    println!("  Low volatility (< {:.4}): {}% of time", 
             vol_percentiles.0,
             rolling_vols.iter().filter(|&&v| v < vol_percentiles.0).count() * 100 / rolling_vols.len());
    println!("  Normal volatility ({:.4} - {:.4}): {}% of time", 
             vol_percentiles.0, vol_percentiles.1,
             rolling_vols.iter().filter(|&&v| v >= vol_percentiles.0 && v <= vol_percentiles.1).count() * 100 / rolling_vols.len());
    println!("  High volatility (> {:.4}): {}% of time", 
             vol_percentiles.1,
             rolling_vols.iter().filter(|&&v| v > vol_percentiles.1).count() * 100 / rolling_vols.len());
    ...
    let d1 = ((spot_price / strike_price).ln() + (risk_free_rate + 0.5 * annualized_vol.powi(2)) * time_to_expiry) 
    ...
    for (multiplier, scenario) in stress_scenarios {
        ...
        println!("    Daily volatility: {:.4} ({:.2}% annualized)", 
                stressed_vol, stressed_vol * (252.0_f64).sqrt() * 100.0);
    ...
    let standardized_residuals: Vec<f64> = returns_data.values.iter()
        .zip(&fitted_volatilities)
        .map(|(ret, vol)| ret / vol)
        .collect();

    let residual_mean = standardized_residuals.iter().sum::<f64>() / standardized_residuals.len() as f64;
    let residual_variance = standardized_residuals.iter()
        .map(|r| (r - residual_mean).powi(2))
        .sum::<f64>() / standardized_residuals.len() as f64;
    ...
    println!("  Min: {:.4}", standardized_residuals.iter().fold(f64::INFINITY, |a, &b| a.min(b)));
    println!("  Max: {:.4}", standardized_residuals.iter().fold(f64::NEG_INFINITY, |a, &b| a.max(b)));
...
mod tests {
    ...
    fn test_garch_example() {
        let result = main();
        assert!(result.is_ok(), "GARCH example should run successfully: {:?}", result);
    }
} 