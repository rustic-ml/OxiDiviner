...
impl ETSModel {
    ...
    pub fn holt_winters_additive(alpha: f64, beta: f64, gamma: f64, period: usize) -> std::result::Result<Self, ESError> {
        Self::new(
            ErrorType::Additive,
            TrendType::Additive,
    ...
    pub fn holt_winters_multiplicative(alpha: f64, beta: f64, gamma: f64, period: usize) -> std::result::Result<Self, ESError> {
        Self::new(
            ErrorType::Additive,
            TrendType::Additive,
...
impl Forecaster for ETSModel {
    ...
    fn fit(&mut self, data: &TimeSeriesData) -> Result<()> {
        ...
        match (self.trend_type, self.seasonal_type) {
            ...
            (TrendType::Additive | TrendType::DampedAdditive, SeasonalType::Additive | SeasonalType::Multiplicative) => {
                ...
                let second_season_avg = data.values[period..2*period].iter().sum::<f64>() / period as f64;
        ...
        for i in start_idx..n {
            ...
            let forecast = match (self.trend_type, self.seasonal_type) {
                ...
                (TrendType::DampedAdditive, SeasonalType::None) => level + self.phi.unwrap() * trend,
                (TrendType::Additive, SeasonalType::Additive) => level + trend + seasonal[s_idx],
                (TrendType::DampedAdditive, SeasonalType::Additive) => level + self.phi.unwrap() * trend + seasonal[s_idx],
                (TrendType::Additive, SeasonalType::Multiplicative) => (level + trend) * seasonal[s_idx],
                (TrendType::DampedAdditive, SeasonalType::Multiplicative) => (level + self.phi.unwrap() * trend) * seasonal[s_idx],
                _ => {
                    return Err(OxiError::from(ESError::UnsupportedModelType(format!(
                        "ETS({},{},{})", 
                        self.error_type, 
            ...
            match (self.trend_type, self.seasonal_type) {
                ...
                (TrendType::Additive, SeasonalType::None) => {
                    ...
                    trend = self.beta.unwrap() * (level - old_level) + (1.0 - self.beta.unwrap()) * trend;
                },
                
                // Damped Trend Method
                (TrendType::DampedAdditive, SeasonalType::None) => {
                    let old_level = level;
                    level = self.alpha * data.values[i] + (1.0 - self.alpha) * (level + self.phi.unwrap() * trend);
                    trend = self.beta.unwrap() * (level - old_level) + (1.0 - self.beta.unwrap()) * self.phi.unwrap() * trend;
                },
        ...
        self.trend = if self.trend_type != TrendType::None { Some(trend) } else { None };
        self.seasonal = if self.seasonal_type != SeasonalType::None { Some(seasonal) } else { None };
...
mod tests {
    ...
    fn test_ets_holt() {
        ...
        for (i, forecast) in forecasts.iter().enumerate() {
            ...
            assert!((forecast - expected).abs() < 0.5, 
                   "Forecast {} expected to be close to {}, but was {}", 
                   i + 1, expected, forecast);
        }
    }
} 