...
impl HoltWintersModel {
    ...
    pub fn new(alpha: f64, beta: f64, gamma: f64, period: usize) -> std::result::Result<Self, ESError> {
        // Validate parameters
        ...
        let name = format!("HoltWinters(α={:.3}, β={:.3}, γ={:.3}, m={})", 
...
impl Forecaster for HoltWintersModel {
    ...
    fn fit(&mut self, data: &TimeSeriesData) -> Result<()> {
        ...
        for i in self.period..n {
            ...
            let old_level = level;
            level = self.alpha * (data.values[i] - seasonal[s_idx]) + (1.0 - self.alpha) * (level + trend);
            trend = self.beta * (level - old_level) + (1.0 - self.beta) * trend;
            seasonal[s_idx] = self.gamma * (data.values[i] - level) + (1.0 - self.gamma) * seasonal[s_idx];
        }
...
mod tests {
    ...
    fn test_holt_winters_forecast() {
        ...
        let timestamps: Vec<DateTime<Utc>> = (0..n)
            .map(|i| Utc.timestamp_opt(now.timestamp() + i as i64 * 86400 * 90, 0).unwrap())
            .collect();
