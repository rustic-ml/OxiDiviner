...
impl From<ESError> for OxiError {
    fn from(e: ESError) -> Self {
        match e {
            ...
            ESError::InvalidAlpha(v) => OxiError::InvalidParameter(format!("Invalid alpha value: {}", v)),
            ESError::InvalidBeta(v) => OxiError::InvalidParameter(format!("Invalid beta value: {}", v)),
            ESError::InvalidGamma(v) => OxiError::InvalidParameter(format!("Invalid gamma value: {}", v)),
            ESError::InvalidPeriod(v) => OxiError::InvalidParameter(format!("Invalid period: {}", v)),
            ESError::NotFitted => OxiError::ModelError("Model has not been fitted yet".into()),
            ESError::InvalidHorizon(v) => OxiError::InvalidParameter(format!("Invalid horizon: {}", v)),
            ESError::InsufficientData { actual, expected } => 
                OxiError::DataError(format!("Insufficient data: {} points provided, {} required", actual, expected)),
            ESError::InvalidDampingFactor(v) => OxiError::InvalidParameter(format!("Invalid damping factor: {}", v)),
            ESError::MissingParameter(param) => OxiError::InvalidParameter(format!("Missing required parameter: {}", param)),
            ESError::UnsupportedModelType(model) => OxiError::ModelError(format!("Unsupported model type: {}", model)),
        }
    }
}
