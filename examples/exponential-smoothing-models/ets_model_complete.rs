...
use oxidiviner_exponential_smoothing::{ETSModel, SimpleESModel, HoltLinearModel, HoltWintersModel, DampedTrendModel};
use std::error::Error;
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    println!("  Damped Trend forecast (10 periods): {:.2?}", damped_forecast);
    ...
    println!("  ETS(A,A,N) forecast (10 periods): {:.2?}", ets_holt_forecast);
    ...
    println!("  Holt-Winters forecast (first 12 periods): {:.2?}", &hw_forecast[..12]);
    ...
    println!("  ETS(A,A,M) forecast (first 12 periods): {:.2?}", &ets_mult_forecast[..12]);
    ...
    println!("  Simple ES forecast (10 periods): {:.2?}", ses_range_forecast);
    
    // ETS simple model
    let mut ets_simple = ETSModel::simple(0.2)?;
    ets_simple.fit(&ranging_data)?;
    let ets_simple_forecast = ets_simple.forecast(10)?;
    println!("  ETS(A,N,N) forecast (10 periods): {:.2?}", ets_simple_forecast);
    ...
    println!("  Damped Trend forecast (10 periods): {:.2?}", damped_volatile_forecast);
    ...
    println!("  ETS(A,Ad,N) forecast (10 periods): {:.2?}", ets_damped_forecast);
...
fn create_trending_data() -> Result<TimeSeriesData, Box<dyn Error>> {
    ...
    let timestamps: Vec<DateTime<Utc>> = (0..100)
        .map(|i| Utc.from_utc_datetime(&base_date.and_hms_opt(0, 0, 0).unwrap().checked_add_signed(chrono::Duration::days(i)).unwrap()))
        .collect();
...
fn create_seasonal_data() -> Result<TimeSeriesData, Box<dyn Error>> {
    ...
    let timestamps: Vec<DateTime<Utc>> = (0..120) // 10 years of monthly data
        .map(|i| Utc.from_utc_datetime(&base_date.and_hms_opt(0, 0, 0).unwrap().checked_add_signed(chrono::Duration::days(30 * i)).unwrap()))
        .collect();
...
fn create_volatile_data() -> Result<TimeSeriesData, Box<dyn Error>> {
    ...
    for i in 0..100 {
        ...
        if value < 50.0 { value = 50.0; } // Set a floor
        values.push(value);
    }
