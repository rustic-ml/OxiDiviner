...
use oxidiviner_exponential_smoothing::{ETSModel, SimpleESModel, HoltLinearModel, DampedTrendModel, HoltWintersModel};
use std::error::Error;
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let scenarios = ["Trending Market", "Sideways Market", "Seasonal Market", "Volatile Market"];
    ...
    for (i, results) in eval_results.iter().enumerate() {
        ...
        for eval in &sorted_results {
            println!("{:<15} {:<10.4} {:<10.4}", eval.model_name, eval.mae, eval.rmse);
        }
        
        // Highlight best model
        println!("\nBest model: {} (Lowest MAE: {:.4})", sorted_results[0].model_name, sorted_results[0].mae);
    }
