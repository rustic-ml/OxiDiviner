...
use oxidiviner_core::{TimeSeriesData, ModelEvaluation};
...
fn main() {
    ...
    let train_trend = TimeSeriesData::new(train_trend_timestamps.clone(), train_trend_values, "train_trend").unwrap();
    
    let test_trend_timestamps = timestamps[train_size..].to_vec();
    let test_trend_values = trend_data.values[train_size..].to_vec();
    let test_trend = TimeSeriesData::new(test_trend_timestamps.clone(), test_trend_values, "test_trend").unwrap();
    
    let train_seasonal_values = seasonal_data.values[0..train_size].to_vec();
    let train_seasonal = TimeSeriesData::new(train_trend_timestamps.clone(), train_seasonal_values, "train_seasonal").unwrap();
    
    let test_seasonal_values = seasonal_data.values[train_size..].to_vec();
    let test_seasonal = TimeSeriesData::new(test_trend_timestamps.clone(), test_seasonal_values, "test_seasonal").unwrap();
    
    let train_y1_values = y1_data.values[0..train_size].to_vec();
    let train_y1 = TimeSeriesData::new(train_trend_timestamps.clone(), train_y1_values, "train_y1").unwrap();
    
    let test_y1_values = y1_data.values[train_size..].to_vec();
    let test_y1 = TimeSeriesData::new(test_trend_timestamps.clone(), test_y1_values, "test_y1").unwrap();
    
    let train_y2_values = y2_data.values[0..train_size].to_vec();
    let train_y2 = TimeSeriesData::new(train_trend_timestamps.clone(), train_y2_values, "train_y2").unwrap();
    
    let test_y2_values = y2_data.values[train_size..].to_vec();
    let test_y2 = TimeSeriesData::new(test_trend_timestamps.clone(), test_y2_values, "test_y2").unwrap();
    ...
    let arima_train_values: Vec<f64> = (0..train_size).map(|i| {
        let trend = (i as f64) * 2.0;
        let noise = rng.gen::<f64>() * 10.0 - 5.0;
        trend + noise
    }).collect();
    
    let arima_test_values: Vec<f64> = (train_size..n).map(|i| {
        let trend = (i as f64) * 2.0;
        let noise = rng.gen::<f64>() * 10.0 - 5.0;
        trend + noise
    }).collect();
    
    let arima_train = TimeSeriesData::new(train_trend_timestamps.clone(), arima_train_values, "arima_train").unwrap();
    let arima_test = TimeSeriesData::new(test_trend_timestamps.clone(), arima_test_values, "arima_test").unwrap();
    
    // Create suitable seasonal data for SARIMA
    let sarima_train_values: Vec<f64> = (0..train_size).map(|i| {
    ...
    let sarima_test_values: Vec<f64> = (0..(n-train_size)).map(|i| {
        let j = i + train_size;
    ...
    let sarima_train = TimeSeriesData::new(train_trend_timestamps.clone(), sarima_train_values, "sarima_train").unwrap();
    let sarima_test = TimeSeriesData::new(test_trend_timestamps.clone(), sarima_test_values, "sarima_test").unwrap();
