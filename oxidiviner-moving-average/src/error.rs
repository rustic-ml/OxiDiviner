...
impl From<MAError> for OxiError {
    fn from(e: MAError) -> Self {
        match e {
            ...
            MAError::InvalidWindowSize(v) => OxiError::InvalidParameter(format!("Invalid window size: {}", v)),
            MAError::TimeSeriesTooShort { actual, expected } => 
                OxiError::DataError(format!("Time series length ({}) must be at least equal to window size ({})", actual, expected)),
            MAError::NotFitted => OxiError::ModelError("Model has not been fitted yet".into()),
            MAError::InvalidHorizon(v) => OxiError::InvalidParameter(format!("Invalid horizon: {}", v)),
        }
    }
}
