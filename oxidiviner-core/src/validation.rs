use crate::{TimeSeriesData, OxiError, Result};
...
impl ValidationUtils {
    ...
    /// # }
    /// ```
    pub fn time_split(data: &TimeSeriesData, train_ratio: f64) -> Result<(TimeSeriesData, TimeSeriesData)> {
    ...
    pub fn accuracy_metrics(
        ...
        let mase = if let Some(baseline) = baseline_naive {
            Some(Self::mean_absolute_scaled_error(actual, predicted, baseline)?)
        } else {
            None
        };
    ...
    fn mean_absolute_percentage_error(actual: &[f64], predicted: &[f64]) -> Result<f64> {
        ...
        for (a, p) in actual.iter().zip(predicted.iter()) {
            if a.abs() > f64::EPSILON {  // Avoid division by zero
                sum += ((a - p) / a).abs() * 100.0;
                count += 1;
            }
        }
    ...
    fn mean_absolute_scaled_error(actual: &[f64], predicted: &[f64], baseline: &[f64]) -> Result<f64> {
