...
impl OHLCVData {
    ...
    pub fn from_csv<P: AsRef<Path>>(path: P, timestamp_format: &str, has_header: bool) -> Result<Self> {
        ...
        for line in lines {
            ...
            if fields.len() < 6 {
                return Err(OxiError::DataError("CSV file must have at least 6 columns: date,open,high,low,close,volume".to_string()));
            }
            ...
            let timestamp = match chrono::NaiveDateTime::parse_from_str(fields[0], timestamp_format) {
                ...
                Err(e) => return Err(OxiError::DataError(format!("Error parsing date '{}': {}", fields[0], e))),
            };
            
            // Parse OHLCV values
            let open_val = fields[1].parse::<f64>().map_err(|e| OxiError::DataError(format!("Error parsing open: {}", e)))?;
            let high_val = fields[2].parse::<f64>().map_err(|e| OxiError::DataError(format!("Error parsing high: {}", e)))?;
            let low_val = fields[3].parse::<f64>().map_err(|e| OxiError::DataError(format!("Error parsing low: {}", e)))?;
            let close_val = fields[4].parse::<f64>().map_err(|e| OxiError::DataError(format!("Error parsing close: {}", e)))?;
            let volume_val = fields[5].parse::<f64>().map_err(|e| OxiError::DataError(format!("Error parsing volume: {}", e)))?;
...
impl TimeSeriesData {
    ...
    pub fn new(timestamps: Vec<DateTime<Utc>>, values: Vec<f64>, name: &str) -> Result<Self> {
        if timestamps.len() != values.len() {
            return Err(OxiError::DataError("Timestamps and values must have the same length".to_string()));
        }
    ...
    pub fn train_test_split(&self, train_ratio: f64) -> Result<(Self, Self)> {
        if train_ratio <= 0.0 || train_ratio >= 1.0 {
            return Err(OxiError::InvalidParameter("Train ratio must be between 0 and 1".to_string()));
        }

        let split_idx = (self.len() as f64 * train_ratio).round() as usize;
        if split_idx == 0 || split_idx >= self.len() {
            return Err(OxiError::InvalidParameter("Invalid split index".to_string()));
        }
